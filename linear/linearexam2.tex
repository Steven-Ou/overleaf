...
\usepackage[letterpaper, margin=0.5in]{geometry} % Minimized margins for max content
...
\definecolor{titlebg}{Blue!70!Black}
\definecolor{thmblue}{Blue!60!Black}
\definecolor{defgreen}{OliveGreen!70!Black}
\definecolor{exred}{Red!70!Black}
\definecolor{imtyellow}{Apricot!80!White}

% --- CUSTOM TCOLORBOX STYLES ---
% Definition Box (Green)
\newtcolorbox{defbox}{
    enhanced,
    colback=defgreen!5!white,
    colframe=defgreen!75!black,
    boxrule=0.5pt,
    fonttitle=\bfseries\sffamily,
    coltitle=white,
    colbacktitle=defgreen!80!black,
...
\newtcolorbox{thmbox}{
    enhanced,
    colback=thmblue!5!white,
    colframe=thmblue!75!black,
    boxrule=0.5pt,
    fonttitle=\bfseries\sffamily,
    coltitle=white,
    colbacktitle=thmblue!80!black,
    attach boxed title to top left={yshift=-2mm, xshift=3mm},
    boxed title style={boxrule=0.5pt, frame code={}},
    title=Theorem,
    sharp corners,
...
\newtcolorbox{exbox}{
    enhanced,
    colback=exred!5!white,
    colframe=exred!75!black,
    boxrule=0.5pt,
    fonttitle=\bfseries\sffamily,
    coltitle=white,
    colbacktitle=exred!80!black,
    attach boxed title to top left={yshift=-2mm, xshift=3mm},
    boxed title style={boxrule=0.5pt, frame code={}},
    title=Example,
    sharp corners,
...
% --- TITLE ---
\title{\textbf{\Huge MATH 231: Cumulative Review Sheet (Ch 1-3)}}
...
\maketitle
\thispagestyle{empty}
\footnotesize % Setting font size for density

% =================================================================
% --- PAGE 1: Linear Systems, Vectors, and Solution Sets ---
% =================================================================
\begin{center}
\textbf{\large --- PAGE 1: Linear Systems, Vectors, and Solution Sets ---}
\end{center}
\hrule

\begin{multicols}{2} % <<< START TWO-COLUMN FOR PAGE 1 >>>
...
The linear system, the vector equation ($x_1 \mathbf{a}_1 + \dots + x_n \mathbf{a}_n = \mathbf{b}$), and the matrix equation ($A\mathbf{x} = \mathbf{b}$) \textbf{all share the same solution set}. $\mathbf{b}$ is in Span$\{\mathbf{a}_1, \dots, \mathbf{a}_n\}$ $\iff A\mathbf{x} = \mathbf{b}$ is consistent.
...
$$
\mathbf{x} = \mathbf{p} + \mathbf{v}_h \quad (\text{Particular} + \text{Homogeneous})
$$
...
$$
\mathbf{x} = \begin{bmatrix} x_1 \\ x_2 \\ x_3 \\ x_4 \end{bmatrix} = \underbrace{\begin{bmatrix} 5 \\ -3 \\ 0 \\ 0 \end{bmatrix}}_{\mathbf{p}} + t \underbrace{\begin{bmatrix} -3 \\ -2 \\ 1 \\ 0 \end{bmatrix}}_{\mathbf{v}_h}
$$
\end{exbox}
...
\end{multicols} % <<< END TWO-COLUMN FOR PAGE 1 >>>

\pagebreak % <<< PAGE BREAK: FRONT/BACK >>>

% =================================================================
% --- PAGE 2: Transformations, Matrix Algebra, and IMT ---
% =================================================================
\begin{center}
\textbf{\large --- PAGE 2: Transformations, Algebra, Determinants, and IMT ---}
\end{center}
\hrule

\begin{multicols}{2} % <<< START TWO-COLUMN FOR PAGE 2 >>>
...
\begin{itemize}
    \item $(A^{-1})^{-1} = A$
    \item $(AB)^{-1} = B^{-1}A^{-1}$ (\textbf{Reversal Rule})
    \item $(A^T)^{-1} = (A^{-1})^T$
...
$$ (\text{\# pivot cols}) + (\text{\# free variables}) = (\text{\# total cols}) $$
\end{thmbox}
...
\end{itemize}
\end{thmbox}
\vfill\null % Pushes content to the top
\columnbreak % Manually break column
...
Let $A$ be a square $n \times n$ matrix. The following statements are all equivalent (all true or all false).
...
\end{multicols} % <<< END TWO-COLUMN FOR PAGE 2 >>>
\end{document}