...
No, the tree diagram from part f no longer applies to this version of experiment. If, we use the tree diagram, we would ended up double counting every pair. This is why we can't apply the tree diagram to this part. 
...
\begin{tikzpicture}[level distance=1.5cm,
  ...
  \node {Start}
    child {node {Pres: A}
      child {node {Tr: B} child {node {Tr: C}}}
      child [dashed] {node {Tr: C} child {node {Tr: B}}} % Redundant path
    }
    child {node {Pres: B}
      child {node {Tr: A} child {node {Tr: C}}}
      child [dashed] {node {Tr: C} child {node {Tr: A}}} % Redundant path
    }
    child {node {Pres: C}
      child {node {Tr: A} child {node {Tr: B}}}
      child [dashed] {node {Tr: B} child {node {Tr: A}}} % Redundant path
    };
\end{tikzpicture}
\end{center}
...
\begin{enumerate}
    \item The first element can be chosen in $n$ ways.
    \item The second element can be chosen in $(n-1)$ ways.
    \item This continues until the $k$-th element, which can be chosen in $(n - (k - 1)) = (n - k + 1)$ ways.
\end{enumerate}

The product of these choices is:
\[ P_{n,k} = n \times (n-1) \times (n-2) \times \dots \times (n-k+1) \]

To express this in factorial notation, we multiply the expression by $\frac{(n-k)!}{(n-k)!}$:
\[ P_{n,k} = \frac{n \times (n-1) \times \dots \times (n-k+1) \times (n-k) \times \dots \times 1}{(n-k)!} \]
\[ P_{n,k} = \frac{n!}{(n-k)!} \]
