\documentclass[12pt]{article}
\usepackage{tcolorbox}
\usepackage{amsmath}


\include{preamble}
%All credit goes towards Dr. Adam Kapelner for providing the preamble and homework template.

\newtoggle{professormode}



\title{MATH 241 Spring 2026 Homework \#7}

\author{Steven Ou} %STUDENTS: write your name here

\iftoggle{professormode}{
\date{Due via Brightspace by 11:59PM Sunday, April $12^{th}$, 2026 \\ \vspace{0.5cm} \small (this document last updated \today ~at \currenttime)}
}

\renewcommand{\abstractname}{Instructions and Philosophy}

\begin{document}
\maketitle

\iftoggle{professormode}{
\begin{abstract}
The path to success in this class is to do many problems. Unlike other courses, exclusively doing reading(s) will not help. Coming to lecture is akin to watching workout videos; thinking about and solving problems on your own is the actual ``working out.''  Feel free to \qu{work out} with others; \textbf{I want you to work on this in groups.}

The problems below are color coded: \ingreen{green} problems are considered \textit{easy} and marked \qu{[easy]}; \inorange{yellow} problems are considered \textit{intermediate} and marked \qu{[harder]}, \inred{red} problems are considered \textit{difficult} and marked \qu{[difficult]} and \inpurple{purple} problems are extra credit. The \textit{easy} problems are intended to be ``giveaways'' if you went to class. Do as much as you can of the others; I expect you to at least attempt the \textit{difficult} problems. 

Bonus points are given as a bonus if the homework is typed using \LaTeX. Links to installing \LaTeX~and programs for compiling \LaTeX~is found on the syllabus. You are encouraged to use \url{https://www.overleaf.com/}. If you are handing in homework this way, read the comments in the code; there is a line to comment out and you should replace my name with yours. The easiest way to use Overleaf is to go to \textit{Menu} and select \textit{Copy Project}. If you are asked to make drawings, you can take a picture of your handwritten drawing and insert them as figures or leave space using the \qu{$\backslash$vspace} command and draw them in after printing or attach them stapled.


\end{abstract}

\thispagestyle{empty}
\vspace{1cm}
NAME: \line(1,0){380}
\clearpage
}

\problem{Please skim through the article found here: \url{https://en.wikipedia.org/wiki/Bayes\%27_theorem}. }



\begin{enumerate}

\easysubproblem{When was Bayes' Theorem published?}\\
Bayes' Theorem was first published posthumously in 1763. It appeared in a paper titled "An Essay Towards Solving a Problem in the Doctrine of Chances". The work was edited and presented to the Royal Society by his friend Richard Price, two years after Thomas Bayes's death. 


\easysubproblem{True or False? Bayes' Theorem is to the theory of probability what the Pythagorean Theorem is to  geometry. }
\\True
\end{enumerate}


\problem{These exercises will review The Law of Total Probability. }

\begin{enumerate}


\inthenotessubproblem{ State The Law of Total Probability.}\\
Suppose the events $B_1, B_2, \dots, B_K$ forms a partition on a sample space $S$ such that $P(B_j) > 0$ for $j=1,2,\dots,k$. Then for every event $A$ in $S$, $$P(A)=\sum_{j=1}^{K}P(B_j)P(A|B_j)$$
\inthenotessubproblem{Prove The Law of Total Probability.}\\
Proof: Since $B_1, B_2, \dots, B_K$ partitions $S$, then $(B_1 \cap A, B_2 \cap A, \dots, B_K \cap A)$ partitions $A$. By the definition of partition, this means $[(B_1 \cap A) \cup (B_2 \cap A) \cup \dots \cup (B_K \cap A) = A]$. By the 3rd Axiom of Probability (for disjoint events): $$P(B_1 \cap A) + \dots + P(B_K \cap A) = P(A)$$Which can also be written as: $\sum_{i=1}^{K}P(B_i \cap A) = P(A)$. By the Basic Conditional Rule, this substitutes to: $$\sum_{i=1}^{K}P(B_i)P(A|B_i) = P(A)$$


\inthenotessubproblem{State Bayes' Theorem.}

Suppose the events $B_1, B_2, \dots, B_k$ forms a partition on a sample space $S$ such that $P(B_j) > 0$ for $j=1,2,\dots,k$. Also suppose that $A$ is any event in $S$ such that $P(A) > 0$. Then for $i=1,2,\dots,k$: $$P(B_i|A) = \frac{P(B_i)P(A|B_i)}{\sum_{j=1}^{k}P(B_j)P(A|B_j)}$$

\inthenotessubproblem{Prove Bayes' Theorem.}\\
$P(B_i|A) = \frac{P(B_i \cap A)}{P(A)}$
$= \frac{P(B_i)P(A|B_i)}{P(A)}$$= \frac{P(B_i)P(A|B_i)}{\sum_{j=1}^{k}P(B_j)P(A|B_j)}$ 
\\By the Basic Conditional Rule, we were able to complete this step :$P(B_i|A) = \frac{P(B_i \cap A)}{P(A)}$, then we substituting the denominator P(A) using the law of total probability, we end up with the last part:$ \frac{P(B_i)P(A|B_i)}{\sum_{j=1}^{k}P(B_j)P(A|B_j)}$.
\inthenotessubproblem{In order to use The Law of Total Probability or Bayes' Theorem, we always need a partition. Explain what is meant by a partition.}\\
A partition fragments the sample space into disjoint pieces (and if you were to "glue" these pieces back together, you would have recovered the entire sample space).

\end{enumerate}

\problem{These exercises will test some of the applications of The Law of Total Probability and Bayes' Theorem.}


\begin{enumerate}

\intermediatesubproblem{Suppose we have a deck of cards from which Player 1 randomly selects a single card (without replacement) and then Player 2 also selects a single card from the deck. Find the probability that Player 2 draws an ace. Does your answer make sense?
 }
\\Creating some events:\\
Let $B_1 = \{\text{Player 1 draws an ace}\}\\$Let $B_2 = \{\text{Player 1 does NOT draw an ace}\}\\$Let $A = \{\text{Player 2 draws an ace}\}$
\\$P(B_1) = \frac{4}{52}$\space |
$P(B_2) = \frac{48}{52}$\space |$P(A|B_1) = \frac{3}{51}$ (One ace is already gone)\space |\\
$P(A|B_2) = \frac{4}{51}$ (All four aces are still in the deck)
\\Using the law of total prob:
$$P(A) = P(B_1)P(A|B_1) + P(B_2)P(A|B_2)$$$$P(A) = \left(\frac{4}{52}\right)\left(\frac{3}{51}\right) + \left(\frac{48}{52}\right)\left(\frac{4}{51}\right)$$$$P(A) = \frac{12}{2652} + \frac{192}{2652}$$$$P(A) = \frac{204}{2652}$$ My answer make senese since the outcome of the unconditional probabilities of both players drawing an ace is exactly the same as if they had drawn first. 
\intermediatesubproblem{Suppose there are bowls, $B_1, B_2$, and $B_3$. Bowl 1 contains two red and four white chips, Bowl 2 contains one red and two white chips and Bowl 3 contains five red and four white chips. Suppose a bowl is chosen at random. What is the probability that we obtain a red chip?
}\\Creating some events:\\
Let $B_1 = \{\text{Bowl 1 is chosen}\}$$\rightarrow P(B_1) = \frac{1}{3}\\$Let $B_2 = \{\text{Bowl 2 is chosen}\}$ $\rightarrow P(B_2) = \frac{1}{3}\\$Let $B_3 = \{\text{Bowl 3 is chosen}\}$ $\rightarrow P(B_3) = \frac{1}{3}\\$Let $A = \{\text{We obtain a red chip}\}$\\ $P(A|B_1) = \frac{2}{6} = \frac{1}{3}$ \space | $P(A|B_2) = \frac{1}{3}$ \space | $P(A|B_3) = \frac{5}{9}$\\Now, using law of total prob:$$P(A) = P(B_1)P(A|B_1) + P(B_2)P(A|B_2) + P(B_3)P(A|B_3)$$$$P(A) = \left(\frac{1}{3}\right)\left(\frac{1}{3}\right) + \left(\frac{1}{3}\right)\left(\frac{1}{3}\right) + \left(\frac{1}{3}\right)\left(\frac{5}{9}\right)$$$$P(A) = \frac{1}{9} + \frac{1}{9} + \frac{5}{27}$$$$P(A) = \frac{3}{27} + \frac{3}{27} + \frac{5}{27}$$$$P(A) = \frac{11}{27}$$The probability that we obtain a red chip is $\frac{11}{27}$.


\intermediatesubproblem{Suppose there are bowls, $B_1, B_2$, and $B_3$. Bowl 1 contains two red and four white chips, Bowl 2 contains one red and two white chips and Bowl 3 contains five red and four white chips. Suppose a bowl is chosen at random and a chip is selected. We learn that the selected chip is red. What is the probability this red chip came from \textbf{Bowl 1}? }
\\Using the event from above:
Now, using Bayes' Theorem:$$P(B_1|A) = \frac{P(B_1)P(A|B_1)}{P(B_1)P(A|B_1) + P(B_2)P(A|B_2) + P(B_3)P(A|B_3)}$$$$P(B_1|A) = \frac{\left(\frac{1}{3}\right)\left(\frac{1}{3}\right)}{\left(\frac{1}{3}\right)\left(\frac{1}{3}\right) + \left(\frac{1}{3}\right)\left(\frac{1}{3}\right) + \left(\frac{1}{3}\right)\left(\frac{5}{9}\right)}$$$$P(B_1|A) = \frac{\frac{1}{9}}{\frac{1}{9} + \frac{1}{9} + \frac{5}{27}}$$$$P(B_1|A) = \frac{\frac{3}{27}}{\frac{3}{27} + \frac{3}{27} + \frac{5}{27}}$$$$P(B_1|A) = \frac{\frac{3}{27}}{\frac{11}{27}}$$$$P(B_1|A) = \frac{3}{11}$$The probability this red chip came from Bowl 1 is $\frac{3}{11}$.

\intermediatesubproblem{Suppose there are bowls, $B_1, B_2, B_3$. Bowl 1 contains two red and four white chips, Bowl 2 contains one red and two white chips and Bowl 3 contains five red and four white chips. Suppose a bowl is chosen at random and a chip is selected. We learn that the selected chip is red. What is the probability this red chip came from \textbf{Bowl 2}? }
\\Since the bowl 2 is similar to bowl 1 because of there probability is equals to 1/3. We going to use the same method as above but using $P(B_2)P(A|B_2)$ , we will still end up getting 3/11 as the probability this red chip came from bowl 2. 

\intermediatesubproblem{Suppose there are bowls, $B_1, B_2, B_3$. Bowl 1 contains two red and four white chips, Bowl 2 contains one red and two white chips and Bowl 3 contains five red and four white chips. Suppose a bowl is chosen at random and a chip is selected. We learn that the selected chip is red. What is the probability this red chip came from \textbf{Bowl 3}? }

The probability of the red chip coming from bowl 3 is 5/11 because we can do 1- 3/11+3/11, and will still get 5/11. Or we can use bayes theorem and $P(B_3)P(A|B_3)$ would be 5/9 * 1/3 and will get $\frac{\frac{5}{27}}{\frac{11}{27}}$, and would still end up with 5/11. 


\intermediatesubproblem{A total of 46 percent of the voters in a certain city classify themselves as
Independents, whereas 30 percent classify themselves as Liberals and 24
percent say that they are Conservatives. In a recent local election, 35 percent
of the Independents, 62 percent of the Liberals, and 58 percent of the
Conservatives voted. A voter is chosen at random. Given that this person
voted in the local election, what is the probability that he or she is an \textbf{Independent}?}
\\Creating some events:
\\Let $B_1 = \{\text{Voter is an Independent}\}$ $\rightarrow P(B_1) = \frac{46}{100}$
\\Let $B_2 = \{\text{Voter is a Liberal}\}$ $\rightarrow P(B_2) = \frac{30}{100}$
\\Let $B_3 = \{\text{Voter is a Conservative}\}$ $\rightarrow P(B_3) = \frac{24}{100}$
\\Let $A = \{\text{Voter voted in the local election}\}$\\
$P(A|B_1) = \frac{35}{100}$ \space | $P(A|B_2) = \frac{62}{100}$ \space | $P(A|B_3) = \frac{58}{100}$
\\Now, using the Bayes Theorem:$$P(B_1|A) = \frac{P(B_1)P(A|B_1)}{P(B_1)P(A|B_1) + P(B_2)P(A|B_2) + P(B_3)P(A|B_3)}$$$$P(B_1|A) = \frac{\left(\frac{46}{100}\right)\left(\frac{35}{100}\right)}{\left(\frac{46}{100}\right)\left(\frac{35}{100}\right) + \left(\frac{30}{100}\right)\left(\frac{62}{100}\right) + \left(\frac{24}{100}\right)\left(\frac{58}{100}\right)}$$$$P(B_1|A) = \frac{\frac{1610}{10000}}{\frac{1610}{10000} + \frac{1860}{10000} + \frac{1392}{10000}}$$$$P(B_1|A) = \frac{\frac{1610}{10000}}{\frac{4862}{10000}}$$$$P(B_1|A) = \frac{1610}{4862}$$$$P(B_1|A) = \frac{805}{2431}$$The probability that he or she is an Independent is $\frac{805}{2431}$.
\intermediatesubproblem{A total of 46 percent of the voters in a certain city classify themselves as
Independents, whereas 30 percent classify themselves as Liberals and 24
percent say that they are Conservatives. In a recent local election, 35 percent
of the Independents, 62 percent of the Liberals, and 58 percent of the
Conservatives voted. A voter is chosen at random. Given that this person
voted in the local election, what is the probability that he or she is a \textbf{Liberal}?}
\\Using the events from above $\frac{\frac{30}{100}*\frac{62}{100}}{\frac{4862}{10000}}=\frac{1860}{4862}= \frac{930}{2431}$. So the probability that he or she is a liberal is $\frac{930}{2431}$.


\intermediatesubproblem{A total of 46 percent of the voters in a certain city classify themselves as
Independents, whereas 30 percent classify themselves as Liberals and 24
percent say that they are Conservatives. In a recent local election, 35 percent
of the Independents, 62 percent of the Liberals, and 58 percent of the
Conservatives voted. A voter is chosen at random. Given that this person
voted in the local election, what is the probability that he or she is a \textbf{Conservative}?}\\
Using the events from above $\frac{\frac{24}{100}*\frac{58}{100}}{\frac{4862}{10000}}=\frac{1392}{4862}= \frac{696}{2431}$. So the probability that he or she is a Conservative is $\frac{696}{2431}$.


\intermediatesubproblem{A total of 46 percent of the voters in a certain city classify themselves as
Independents, whereas 30 percent classify themselves as Liberals and 24
percent say that they are Conservatives. In a recent local election, 35 percent
of the Independents, 62 percent of the Liberals, and 58 percent of the
Conservatives voted. A voter is chosen at random. What percent of voters participated in the local election?}

Using the events from above, the total percentage of voters who participated is just the total probability of event $A$ (the denominator we already calculated).
$P(A) = \frac{4862}{10000} = 48.62\%$.
So $48.62\%$ of voters participated in the local election.

\hardsubproblem{An ectopic (abnormal) pregnancy is twice as likely to develop when the pregnant
woman is a smoker as it is when she is a nonsmoker. If 32 percent of women
of childbearing age are smokers, what percentage of women having ectopic
pregnancies are smokers?}\\
Creating some events:\\Let $B_1 = \{\text{Woman is a smoker}\}$ $\rightarrow P(B_1) = \frac{32}{100}$\\Let $B_2 = \{\text{Woman is a nonsmoker}\}$ $\rightarrow P(B_2) = \frac{68}{100}$\\Let $A = \{\text{Woman has an ectopic pregnancy}\}$ let the probability for a nonsmoker be $x$. Then:$P(A|B_1) = 2x$  |  $P(A|B_2) = x$\\Now, using Bayes' Theorem:$$P(B_1|A) = \frac{P(B_1)P(A|B_1)}{P(B_1)P(A|B_1) + P(B_2)P(A|B_2)}$$$$P(B_1|A) = \frac{\left(\frac{32}{100}\right)(2x)}{\left(\frac{32}{100}\right)(2x) + \left(\frac{68}{100}\right)(x)}$$$$P(B_1|A) = \frac{\frac{64x}{100}}{\frac{64x}{100} + \frac{68x}{100}}$$$$P(B_1|A) = \frac{\frac{64x}{100}}{\frac{132x}{100}}$$$$P(B_1|A) = \frac{64x}{132x}$$$$P(B_1|A) = \frac{64}{132}$$$$P(B_1|A) = \frac{16}{33}$$The percentage of women having ectopic pregnancies who are smokers is $\frac{16}{33}$.

\hardsubproblem{Ninety-eight percent of all babies survive delivery. However, 15 percent
of all births involve Cesarean (C) sections, and when a C section is
performed, the baby survives 96 percent of the time. If a randomly chosen pregnant woman does not have a C section, then what is the probability that her
baby survives?\\
}Creating some events:\\Let $B_1 = \{\text{Birth involves a C section}\}$ $\rightarrow P(B_1) = \frac{15}{100}$\\Let $B_2 = \{\text{Birth does not involve a C section}\}$ $\rightarrow P(B_2) = \frac{85}{100}$\\Let $A = \{\text{Baby survives delivery}\}$ $\rightarrow P(A) = \frac{98}{100}$\\$P(A|B_1) = \frac{96}{100}$  |  $P(A|B_2) = x$\\Now, using the Law of Total Probability:$$P(A) = P(B_1)P(A|B_1) + P(B_2)P(A|B_2)$$$$\frac{98}{100} = \left(\frac{15}{100}\right)\left(\frac{96}{100}\right) + \left(\frac{85}{100}\right)(x)$$$$\frac{98}{100} = \frac{1440}{10000} + \frac{85x}{100}$$$$\frac{9800}{10000} = \frac{1440}{10000} + \frac{8500x}{10000}$$$$9800 = 1440 + 8500x$$$$8360 = 8500x$$$$x = \frac{8360}{8500}$$$$x = \frac{418}{425}$$The probability that her baby survives is $\frac{418}{425}$ (or approximately $98.35\%$).

\hardsubproblem{ To become an actuary, this involves taking a probability exam known as Exam P. (If you find probability interesting or you're unsure of careers in math, then take a look at this website: \url{https://www.beanactuary.org/why-actuarial-science/} and this site: \url{https://www.ezrapenland.com/resources/actuarial-salary-survey/}.) This is a sample question from Exam P: An insurance company examines its pool of auto insurance customers and gathers the following information:
\begin{enumerate}
    \item All customers insure at least one car.
    \item 70\% of the customers insure more than one car.
    \item 20\% of the customers insure a sports car.
    \item Of those customers who insure more than one car, 15\% insure a sports car.
\end{enumerate}
Calculate the probability that a randomly selected customer insures exactly one car and that car is not a sports car.}

Creating some events:\\Let $B_1 = \{\text{Customer insures exactly one car}\}$ $\rightarrow P(B_1) = \frac{30}{100}$\\Let $B_2 = \{\text{Customer insures more than one car}\}$ $\rightarrow P(B_2) = \frac{70}{100}$\\Let $A = \{\text{Customer insures a sports car}\}$ $\rightarrow P(A) = \frac{20}{100}$\\We are given $P(A|B_2) = \frac{15}{100}$.\\First, using the Law of Total Probability to find $P(A|B_1)$:$$P(A) = P(B_1)P(A|B_1) + P(B_2)P(A|B_2)$$$$\frac{20}{100} = \left(\frac{30}{100}\right)P(A|B_1) + \left(\frac{70}{100}\right)\left(\frac{15}{100}\right)$$$$\frac{2000}{10000} = \left(\frac{30}{100}\right)P(A|B_1) + \frac{1050}{10000}$$$$\frac{950}{10000} = \left(\frac{30}{100}\right)P(A|B_1)$$$$P(A|B_1) = \frac{950}{10000} \cdot \frac{100}{30}$$$$P(A|B_1) = \frac{95}{300} = \frac{19}{60}$$
$P(A|B_1)$is the probability of all the people who only insure one car, are driving a sport cars, so to find the probablitiy of exactly one car and that car is not a sport car, we subtract 19/60 from 1 and we will get 41/60.
Now, using the conditional probability multiplication rule:$$P(B_1 \cap A^c) = P(B_1)P(A^c|B_1)$$$$P(B_1 \cap A^c) = \left(\frac{30}{100}\right)\left(\frac{41}{60}\right)$$$$P(B_1 \cap A^c) = \frac{1230}{6000}$$$$P(B_1 \cap A^c) = \frac{41}{200}$$The probability that a randomly selected customer insures exactly one car and that car is not a sports car is $\frac{41}{200}$ (or $20.5\%$).

\easysubproblem{This is also a sample question from Exam P: An insurance company issues life insurance policies in three separate categories: standard, preferred, and ultra-preferred. Of the company’s policyholders, 50\% are standard, 40\% are preferred, and 10\% are ultra-preferred. Each standard policyholder has probability 0.010 of dying in the next year, each preferred policyholder has probability 0.005 of dying in the next year, and each ultra-preferred policyholder has probability 0.001 of dying in the next year. A policyholder dies in the next year. What is the probability that the deceased policyholder was ultra-preferred?}
\\Creating some events:
\\Let $B_1 = \{\text{Policyholder is standard}\}$ $\rightarrow P(B_1) = \frac{50}{100}$\\Let $B_2 = \{\text{Policyholder is preferred}\}$ $\rightarrow P(B_2) = \frac{40}{100}$\\Let $B_3 = \{\text{Policyholder is ultra-preferred}\}$ $\rightarrow P(B_3) = \frac{10}{100}$\\Let $A = \{\text{Policyholder dies in the next year}\}$\\$P(A|B_1) = \frac{10}{1000}$  |  $P(A|B_2) = \frac{5}{1000}$  |  $P(A|B_3) = \frac{1}{1000}$\\Now, using Bayes' Theorem:$$P(B_3|A) = \frac{P(B_3)P(A|B_3)}{P(B_1)P(A|B_1) + P(B_2)P(A|B_2) + P(B_3)P(A|B_3)}$$$$P(B_3|A) = \frac{\left(\frac{10}{100}\right)\left(\frac{1}{1000}\right)}{\left(\frac{50}{100}\right)\left(\frac{10}{1000}\right) + \left(\frac{40}{100}\right)\left(\frac{5}{1000}\right) + \left(\frac{10}{100}\right)\left(\frac{1}{1000}\right)}$$$$P(B_3|A) = \frac{\frac{10}{100000}}{\frac{500}{100000} + \frac{200}{100000} + \frac{10}{100000}}$$$$P(B_3|A) = \frac{\frac{10}{100000}}{\frac{710}{100000}}$$$$P(B_3|A) = \frac{10}{710}$$$$P(B_3|A) = \frac{1}{71}$$The probability that the deceased policyholder was ultra-preferred is $\frac{1}{71}$.

\end{enumerate}




\problem{In this exercise, we will study the Monty Hall problem from both a conditional and visual perspective.}

\begin{enumerate}
\hardsubproblem{Suppose you are a contestant on a game show and you have to choose between 3 closed doors. Whatever is behind your chosen door, is your prize. Behind one door is a brand new car (which is what you would like to have) and behind the other two doors are goats. The host of the show Monty knows where the car and goats are located, but you, being the contestant, does not. That is, from your perspective, the placement of the car and goats are random. You now select a door. Before Monty opens up your selected door, he opens up a different door which reveals a goat. (Note that Monty will never open up your door first and he will never open up the door with the car first. If he has multiple eligible doors to choose from, then he selects a door at random). Monty then asks you if you would like to keep your door or switch doors. What should you do and why? For concreteness, suppose we chose Door 1 and Monty opens up Door 3 to reveal a goat. What is the probability that the car is behind Door 2? What is the probability that the car is behind Door 1?}

\begin{tcolorbox}
    This question is difficult so I'll start you off. Let
    \begin{align*}
        B_1 &= \{ \text{the car is behind Door} ~ 1 \}, \\
        B_2 &= \{ \text{the car is behind Door} ~ 2 \}, \\
        B_3 &= \{ \text{the car is behind Door} ~ 3 \}, \\
        \text{and} ~ A &= \{ \text{Monty opens up Door 3 which reveals a goat} \}
    \end{align*}
    Find $P(B_2 | A)$ and find $P(B_1 |A)$. 
\end{tcolorbox}
$P(A|B_1) = \frac{1}{2}$ | $P(A|B_2) = 1$ | $P(A|B_3) = 0$
\\Now, using Bayes' Theorem to find $P(B_2|A)$:$$P(B_2|A) = \frac{P(B_2)P(A|B_2)}{P(B_1)P(A|B_1) + P(B_2)P(A|B_2) + P(B_3)P(A|B_3)}$$$$P(B_2|A) = \frac{\left(\frac{1}{3}\right)(1)}{\left(\frac{1}{3}\right)\left(\frac{1}{2}\right) + \left(\frac{1}{3}\right)(1) + \left(\frac{1}{3}\right)(0)}$$$$P(B_2|A) = \frac{\frac{1}{3}}{\frac{1}{6} + \frac{1}{3} + 0}$$$$P(B_2|A) = \frac{\frac{1}{3}}{\frac{1}{6} + \frac{2}{6}}$$$$P(B_2|A) = \frac{\frac{1}{3}}{\frac{3}{6}}$$$$P(B_2|A) = \frac{\frac{1}{3}}{\frac{1}{2}}$$$$P(B_2|A) = \frac{2}{3}$$Now, using Bayes' Theorem to find $P(B_1|A)$:$$P(B_1|A) = \frac{P(B_1)P(A|B_1)}{P(B_1)P(A|B_1) + P(B_2)P(A|B_2) + P(B_3)P(A|B_3)}$$(Since we already calculated the denominator to be $\frac{1}{2}$)$$P(B_1|A) = \frac{\left(\frac{1}{3}\right)\left(\frac{1}{2}\right)}{\frac{1}{2}}$$$$P(B_1|A) = \frac{\frac{1}{6}}{\frac{1}{2}}$$$$P(B_1|A) = \frac{1}{3}$$The probability that the car is behind Door 2 is $\frac{2}{3}$, and the probability that the car is behind Door 1 is $\frac{1}{3}$. You should switch doors because your probability of winning doubles from $\frac{1}{3}$ to $\frac{2}{3}$.

\easysubproblem{Watch the video provided here which creates a diagram: \url{https://www.youtube.com/watch?v=cphYs1bCeDs}. You do not have to write anything for this question.}

\easysubproblem{The Monty Hall problem is called a  \textit{veridical paradox}. A veridical paradox produces a result that appears counter to intuition, but is demonstrated to be true nonetheless. The paradox comes in from the fact that at the moment Monty opens up Door 3, there are only two configurations left: either Door 1 has the car and Door 2 has the goat OR Door 1 has the goat and Door 2 has the car. Since there are only two configurations, we think there is a 50-50 shot. The problem is, we are not in a simple sample space and this is counter intuitive. Many people have a tough time in believing that we should switch doors despite seeing the math above. To understand why this makes sense intuitively, here are two comments from the YouTube video above: \\

@rogerbodey9475: \qu{The simplest solution: \\
                                1. If you swap doors, you will always get other object (ie goat to car, car to goat). \\
                                2. Two-thirds of the time you will pick a goat initially.
Need I say more?} \\

@cslack813: \qu{if you chose a goat initially Monty HAS TO CHOOSE THE OTHER GOAT. Therefore this needs to be factored into the decision to switch and it’s not 50/50 despite the appearance}. \\

After reading these comments, does the Monty Hall problem seem less paradoxical?}
\\Yes, after reading these comments it seems like the Monty Hall problem seems less paradoxical!

\newpage
\hardsubproblem{Consider the following modified version of the Monty Hall Problem: Suppose you are a contestant on a game show and you have to choose between 100 closed doors. Whatever is behind your chosen door, is your prize. Behind one door is a brand new car and behind the other 99 doors are goats. The host of the show, Monty, knows where the car and goats are located, but you, the contestant, does not. Suppose you choose a door at random. Before Monty opens up your chosen door, he opens up 98 other doors which reveals all goats. Note that while opening up these 98 other doors, Monty will never open your initial door and will never open the door with the car. At this point, there are two doors left. Monty then asks you if you would like to keep your door or if you would like to switch doors. Find the probability you win the car by switching. }
\\Listing out events:
\\Let $B_1 = \{\text{Car is behind Door 1}\}$ $\rightarrow P(B_1) = \frac{1}{100}$\\Let $B_2 = \{\text{Car is behind Door 2}\}$ $\rightarrow P(B_2) = \frac{1}{100}$\\Let $A = \{\text{Monty opens Doors 3 through 100}\}$\\$P(A|B_1) = \frac{1}{99}$  |  $P(A|B_2) = 1$  |  $P(A|B_{\text{any other door}}) = 0$\\Now, using Bayes' Theorem:$$P(B_2|A) = \frac{P(B_2)P(A|B_2)}{P(B_1)P(A|B_1) + P(B_2)P(A|B_2)}$$$$P(B_2|A) = \frac{\left(\frac{1}{100}\right)(1)}{\left(\frac{1}{100}\right)\left(\frac{1}{99}\right) + \left(\frac{1}{100}\right)(1)}$$$$P(B_2|A) = \frac{\frac{1}{100}}{\frac{1}{9900} + \frac{1}{100}}$$$$P(B_2|A) = \frac{\frac{1}{100}}{\frac{1}{9900} + \frac{99}{9900}}$$$$P(B_2|A) = \frac{\frac{1}{100}}{\frac{100}{9900}}$$$$P(B_2|A) = \frac{\frac{1}{100}}{\frac{1}{99}}$$$$P(B_2|A) = \frac{99}{100}$$The probability you win the car by switching is $\frac{99}{100}$.
\end{enumerate}



\end{document}
