...
\\\\The second graph, which is the graph from part f, is the best, because it has the most spaces between the 1 and -1. (Maximum Margin)

\easyquestion{Intuitively explain what is meant by \textit{margin}.}
\\\\Margin means the shortest distance from the hyperplane to the closest data points of any class. 

\easyquestion{Mathematically, how do we define \textit{margin}?}
\\\\Margin is mathematically defined as the separating hyperplane $w*x+b=0$. %For a linearly separable dataset, we can scale w and b such that the points closest to the hyperplane satisfy $\lvert w*x_i+b \rvert =$1. The distance from a point $x_i$ to the hyperplane $w*x_i+b=0$ is given by $\frac{\lvert w*x_i+b \rvert}{\lvert\lvert w \rvert\rvert}$. For the support vectors, this distance is $\frac{1}{\lvert\lvert w \rvert\rvert}$.%
...
\\\\ We could achieve an infinitely large margin by simply scaling the weight vector w to be infinitesimally small.

\easyquestion{Explain why we need the constraint $\forall i, y_i(\vect{w}^T \vect{x_i} + b) \geq 0$.}
\\\\We need the constraint $\forall i, y_i(\vect{w}^T \vect{x_i} + b) \geq 0$ because it a way of stating that all data points in the training set must be correctly classified by the hyperplane defined by $\vect{w}$ and b.
\end{enumerate}
\section*{Problem 3}
...
\end{align*}
\end{tcolorbox}}
\\\\The first formula maximizes the margin $\gamma(\vect{w}, b)$ while ensuring all data points are correctly classified. The second formula fixes the margin to 1 and instead minimizes $\vect{w}^T\vect{w}$, which is equivalent since $\gamma = \frac{1}{|\vect{w}|}$. So minimizing $|\vect{w}|^2$ maximizes the margin. Therefore, both are equivalent ways of stating the same optimization problem, up to a rescaling of $\vect{w}$ and $b$.
...
\end{align*}
\end{tcolorbox} }\\\\
Yes, they are equivalent for optimization because $\frac{1}{2}$ is a constant scalar. Minimizing either leads to the same solution.
Using $\frac{1}{2}$ is conventional in optimization for easier gradient computation (it cancels the 2 when differentiating).
...
\[
\begin{aligned}
\underset{\vect{w}, b} {\text{min}} \{\frac{1}{2} \|\vect{w}\|^2 \} \\
\text{subject to} \quad & y_i(\vect{w}^T \vect{x_i} + b) \geq 1 \quad \text{for all } i = 1, 2, \dots, n
\end{aligned}
\]
\end{tcolorbox}
...
The constraint $\underset{\vect{i}}{\text{min}}|\vect{w}^T\vect{x}_i+b| = 1$. The closest point to the hyperplane has margin 1. Combined with correct classification, this implies that: $y_i(\vect{w}^T\vect{x_i} +b \geq 1 ) \forall i$, so the margin is exactly for the support vectors and larger for others, just like in the primal SVM. 

\easyquestion{Interpret what the primal form (of the hard margin) SVM is asking us to do graphically.}\\\\
The primal SVM tries to find the hyperplane that separates the two classes with the largest margin. It places the decision boundary halfway between the closest points of both classes (support vectors) and maximizes that gap.


\easyquestion{What technique do we use to solve the optimization problem posed in the primal form?}
\\\\We use Quadratic Programming (QP) or Subgradient methods in more complex or large-scale settings.
...
\section*{Iteration 1:}
Is $y_i(\vec{w}^T \vec{x}_i + b) \geq 1$ ? \\
Is $-1\left(\begin{bmatrix} 1 & 1 \end{bmatrix} \begin{bmatrix} 1 \\ 0 \end{bmatrix} + 0\right) \geq 1$ ? \\
$-1(1 + 0 + 0) \geq 1$ ? No! \\
\vspace{1em} % Add some vertical space
$\vec{w}_{\text{new}} = \begin{bmatrix} 1 \\ 1 \end{bmatrix} - 0.001 \left(\begin{bmatrix} 1 \\ 1 \end{bmatrix} - \begin{bmatrix} 0 \\ 0 \end{bmatrix}\right)$ \\
$= \begin{bmatrix} 1 \\ 1 \end{bmatrix} - 0.001 \begin{bmatrix} 2 \\ 1 \end{bmatrix}$ \\ % Transcribing the intermediate step shown in the image
$= \begin{bmatrix} 0.998 \\ 0.999 \end{bmatrix}$ \\
\vspace{1em}
$b_{\text{new}} = 0 + 0.001(-1)$ \\
$= -0.001$
...
\section*{Iteration 2:}
Is $y_i(\vec{w}^T \vec{x}_i + b) \geq 1$ ? \\
Is $-1\left([0.998 \ 0.999]\begin{bmatrix} 2 \\ 0 \end{bmatrix} - 0.001\right) \geq 1$ ? \\
$-1(1.996 - 0.001) \geq 1$ ? No! \\
\vspace{1em}
$\vec{w}_{\text{new}} = \begin{bmatrix} 0.998 \\ 0.999 \end{bmatrix} - 0.001 \left(\begin{bmatrix} 0.998 \\ 0.999 \end{bmatrix} - \begin{bmatrix} 2 \\ 0 \end{bmatrix}\right)$ \\
$= \begin{bmatrix} 0.995002 \\ 0.998001 \end{bmatrix}$ \\
\vspace{1em}
$b_{\text{new}} = -0.001 + 0.001(-1)$ \\
$= -0.002$
...
\easyquestion{Suppose the optimal hyperplane for a  dataset $\mathds{D}$ is given by $\frac{3}{17}x_1 + \frac{5}{17} x_2 - \frac{23}{17} = 0$. Predict the label for the new datapoint $\vect{x}_{\text{new}} = [4 ~~
 2.5]^T$}
\\\\Plug in $\vect{x}_{\text{new}}=\frac{3}{17}(4) + \frac{5}{17}(2.5) - \frac{23}{17} = 0.08824 > 0$ so label is $+1$
\end{enumerate}
...
Hard margin requires that all points be perfectly separated with no violations. For non-linearly separable data, this is impossible, so the constraints can't be satisfied — no solution exists.

\easyquestion{In the case of the soft margin SVM, what is the optimization problem we aim to solve?}
\\\\
$
\underset{\vect{w},\, b,\, \boldsymbol{\xi}}{\text{min}} \frac{1}{2} \|\vect{w}\|^2 + C \sum_i \xi_i \quad \text{subject to} \quad y_i(\vect{w}^T \vect{x}_i + b) \geq 1 - \xi_i, \quad \xi_i \geq 0 \quad \text{for all } i
$\\
\easyquestion{In the case of the soft margin SVM, what does $\xi_i$ represent?}\\\\
It represents a measure of how much room or slack a point I needs for violating the condition $\forall i$, $y_i(\vect{w}^T \vect{x}_i + b) \geq1$. 

\intermediatequestion{In the graph below, a particular point has been circled in green. For that point, sketch  where $\xi_i$ is. }
\begin{figure}[H]
    \centering
\includegraphics[width=0.75\linewidth]{svm_1.png}
\end{figure}

\intermediatequestion{In the graph below, a particular point has been circled in green. For that point, sketch  where $\xi_i$ is. }
\begin{figure}[H]
    \centering
    \includegraphics[width=.75\linewidth]{svm_2.png}
\end{figure}
...
\begin{figure}[H]
    \centering
\includegraphics[width=0.25\linewidth]{svm_4.png}
\end{figure}
}
\end{enumerate}
 I see that the margin is wider in the second graph so the c must be lower. Hence, the bottom graph is $c =0.01$ and the top is $c= 100$.
...
In high dimensions, points become more linearly separable due to the concentration of measure. Example: In 2D, XOR is not linearly separable; in higher-dimensional feature space, it becomes separable via kernel tricks.

\easyquestion{What is the Blessing of Non-Uniformity?}
\\\\Real-world data is not uniformly distributed — it's clustered or structured, which makes learning easier. SVMs exploit this structure for better generalization. 
...
If cancer were random, the plot (tumor size vs. cell density) would show no clear separation between cancerous and non-cancerous — points would be scattered without patterns, making classification impossible. Another way of saying it no visible clustering.
