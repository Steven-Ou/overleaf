...
A column vector is a mathematical object that represents a point or direction in space. This is how we denote a column vector: v=$
\begin{bmatrix}
    v1\\v2\\.\\.\\.\\v_{n}
\end{bmatrix}
$.
...
\\\\Two vectors are equal to each other when they have the same magnitude and direction.
...
\begin{center}
\centering
\includegraphics[width=0.3\textwidth]{add.png} 
\end{center}
}
...
$\begin{Vmatrix}
    v_1
\end{Vmatrix} \approx 4.12 \\ \begin{Vmatrix}
    v_2
\end{Vmatrix} \approx 2.24\\
\begin{Vmatrix}
    v_3
\end{Vmatrix} =4\\
\begin{Vmatrix}
    v_4
\end{Vmatrix} \approx 8.66 
$
\vspace{2mm}
...
\easysubproblem{If $\vect{u}^T \vect{v} < 0$ then what can we say about the angle between $\vect{u}$ and $\vect{v}$?}\\\\
The angle $\theta$ between the vectors is obtuse.
\vspace{2mm}
\intermediatesubproblem{Prove that $\vect{u}^T \vect{v} = \vect{v}^T \vect{u}$}. \\\\
 Both are scalar sums: $\sum\vect{u_iv_i}=\sum\vect{v_iu_i}$, so they are always equal- the dot product is commutative. 
...
$\vect{u}^T \vect{v}$ = (1 * 5) + (2*-2) + (-1*1) = 0. Thus, they are orthogonal.  
...
\\\\$proj_u(v)=\frac{(\vect{u^T\vect{v}})}{\begin{Vmatrix}u
\end{Vmatrix}^2}\vect{u}$
\vspace{2mm}
...
$proj_u(v) = \begin{bmatrix}
    4.75,0.475
\end{bmatrix}$
\begin{center}
\centering
\includegraphics[width=0.5\textwidth]{sk.png} 
\end{center}
...
\\\\They represent the same line; ax+by =c can be rearranged into slope-intercept form. 
\vspace{2mm}

\easysubproblem{In $\mathbb{R}^2$ explain how $ax + by = c$ is the same as $\vect{w}^T \vect{x} + b = 0$.}
\\\\Same idea  - W = [a,b] is a normal vector to the line, x =[x,y] this is any point on the plane, b = -c. 
\vspace{35mm}
...
\\\\
It's a hyperplane, a subspace of dimension n-1. 
\vspace{2mm}


\intermediatesubproblem{Show for any hyperplane  $\vect{w}^T \vect{x} + b = 0$, that $\vect{w}$ is always orthogonal to the hyperplane.}
\\\\
Any direction along the hyperplane keeps $\vect{w^T\vect{x}}$ constant, so w must be perpendicular to any vector in the plane. 
...
-25\\
100 
\end{bmatrix}$ and $b = 0$. Sketch the hyperplane (line) below. }
