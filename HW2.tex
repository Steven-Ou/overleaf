...
\centerline{1st: Convert input data like images into numbers and organize them for training datasets.}
\\\\
\centerline{2nd: Decide the number of layers, neurons per layer, and activation function.}
\\\\
\centerline{3rd: Use gradient descent to update weights and biases so the network gets better at making predictions. }
\\\\
\centerline{4th: Test the model on validation data. Make changes if needed.}
