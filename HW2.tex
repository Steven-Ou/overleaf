...
\\\\ 
Perceptron is a neural network model that takes multiple inputs, it also produces binary numbers based on the inputs. The input could be represented as a vector $x=x_1,x_2,...,x_n$. The weights of each input $x_i$ are associated with a weight $w_i$. The weight determines the importance of each input. The perceptron also has a bias $b$, which is a threshold for the model to shift the condition of the final choice. The perceptron computes a weighted summation of the inputs, including the bias using the formula: $z = \sum_{i=1}^{n}{w_ix_i +b}$. Then using z we would pass it into an activation function, these inputs correspond to a characteristic of the result data if z $\geq$ 0 are not something/choices you wanted to make or do, and z $\geq$1 if the inputs are something/choices you wanted to make or to do.
\vspace{5mm}
